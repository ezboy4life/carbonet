...
class Calculator {
  static double calculateCalories(List<AlimentoIngerido> alimentos) {
    throw UnimplementedError();
    // TODO: colocar o cálculo de calorias
  }

  static double calulateCarbos(List<AlimentoIngerido> alimentos) {
    double total = 0;
    for (var alimento in alimentos) {
      total += (alimento.qtdIngerida * alimento.alimentoReferencia.carbosPorPorcao) / alimento.alimentoReferencia.gramasPorPorcao;
    }
    return total;
  }
  ...
  static int calculateInsulinDosage(double carbs, int currentBloodSugar, int minIdeal, int maxIdeal, int insulinRatio) {
    ...
    int bloodSugarIncreaseByCarbs = (carbs * (mgOfGlucoseOneInsulinUnitLowers / insulinRatio)).round(); // 50 mg glicose são cobertos por uma dose de insulina, que cobre 15g (por exemplo); logo, p/ saber quantos mg de glicose vem de cada g de CHO, dividimos a subida da glicose pela razão insulina-cho.
    ...
    } else {  // minIdeal <= expectedBloodSugar <= maxIdeal
